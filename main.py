...
# which have an ancestor -- for example ndb.Key(User, ..., UserIndex, ...) --
# which is slightly more complex than the keys we have seen so far.

# We define an extra model User to hold all the data for the ancestors being
...
# that a User entity exists to be used as an ancestor, we provide a method
# UserInsert to allow API users to create or update parent objects.
...
class UserIndex(EndpointsModel):
  ...
  #  ndb.Key(User, _parent, UserIndex, _id)
  ...
  def SetKey(self):
    ...
    if self._parent is not None and self._id is not None:
      key = ndb.Key(User, self._parent, UserIndex, self._id)
  ...
  def ParentSet(self, value):
    ...
    if ndb.Key(User, value).get() is None:
      # If the User key does not correspond to an entity in the datastore,
    ...
    self._endpoints_query_info.ancestor = ndb.Key(User, value)
  ...
  def IdSet(self, value):
    ...
    self._id = value+str(random.getrandbits(64))
...
class Tweet(EndpointsModel):
  # These values are placeholders to be used when a key is created; the _parent
  # will be used as the ancestor and the _id as the ID. For example:
  #  ndb.Key(User, _parent, Tweet, _id)
  # Since these values will be set by alias properties which are not set
  # simultaneously, we need to hold them around until both are present before we
  # can create a key from them.
  _parent = None
  ...
  def SetKey(self):
    # Can only set the key if both the parent and the child ID are set.
    if self._parent is not None and self._id is not None:
      key = ndb.Key(User, self._parent, Tweet, self._id)
      # Will set the key and attempt to update the entity if it exists.
      self.UpdateFromKey(key)
...
class MyApi(remote.Service):
  ...
  # the ProtoRPC message schema for User is a single field "name", this will
  # be all that is contained in the request and the response.
  @User.method(path='user', http_method='POST',
                   name='user.insert')
  def UserInsert(self, my_parent):
    # Though we don't actively change the model passed in, the value of updated
    # is set to the current time. No check is performed to see if the User
    # entity already exists, since the values other than the name (set in the
    # key) are not relevant.
    my_parent.put()
    return my_parent

  # Since we require Tweet instances also have a User ancestor, we include
  # "parent" in the request path by setting path='userindex/{parent}'. Since "id"
  ...
  @UserIndex.method(path='userindex/{parent}', http_method='POST',
                  name='userindex.insert')
  def UserIndexInsert(self, my_parent):
    # Though we don't actively change the model passed in, the value of updated
    # is set to the current time. No check is performed to see if the User
    # entity already exists, since the values other than the name (set in the
    # key) are not relevant.
    old_array = my_parent.followers
    new_array = []
    length = 0
    if len(my_parent.followers) >= 5000:
        length = int(math.ceil(len(my_parent.followers)/5000))
    if length == 0:
        length = 1
    for i in range(0,length):
        new_array = old_array[i*5000:(i+1)*5000-1]
        my_parent.followers = new_array
        my_parent.id = my_parent.parent+str(random.getrandbits(64))
        my_parent.put()
    return my_parent

  @Tweet.method(path='tweet/{parent}', http_method='POST',
                  name='tweet.insert')
  def TweetInsert(self, my_model):
    ...
    # UserIndex.SetKey must have been called.
    ...
    if my_model.from_datastore:
      raise endpoints.BadRequestException(
          'UserIndex %s with parent %s already exists.' %
          (my_model.id, my_model.parent))
    my_model.put()
    return my_model

  # To make sure queries have a specified ancestor, we use the alias property
  # "parent" which we defined on UserIndex and specify query_fields equal to
  # ('parent',). To specify the parent in the query, it is included in the path
  # as it was in UserIndexInsert. So no query parameters will be required, simply
  # a request to
  #   .../userindexs/someparent
  # where ... is the full path to the API.
  @UserIndex.query_method(query_fields=('parent',),
                        path='userindexes/{parent}', name='userindex.list')
  def UserIndexList(self, query):
    return query

  @UserIndex.query_method(query_fields=('followers',),
                        path='getfollower/{followers}', name='userindex.follower')
  def GetFollower(self, query):
    return query

  @Tweet.query_method(query_fields=('parent','limit'),
                        path='tweets/{parent}', name='tweet.list')
